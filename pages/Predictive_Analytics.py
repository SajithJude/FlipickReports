...
df_enrollment_metrics.columns[2] = [c.replace('Learner_Email', 'Email_Id') for c in df_enrollment_metrics.columns]
...
df = pd.merge(df, df_enrollment_metrics, on=['Learner_Name', 'Learner_Email'])
