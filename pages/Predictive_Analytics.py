...
st.write(df_activity.iloc[:, 3])
# Merge the dataframes
# Merge the dataframes using iloc to select columns 1 and 2
df = pd.merge(df_activity, df_levelwise_assessment, on=[df_activity.iloc[:,2], df_activity.iloc[:,3]])
df = pd.merge(df, df_enrollment_metrics, on=[df_activity.iloc[:,2], df_activity.iloc[:,3]])
